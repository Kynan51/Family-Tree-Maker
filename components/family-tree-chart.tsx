...
function MemberCard({ member }: { member: FamilyMember }) {
  ...
  return (
    ...
    >
      <Card className="w-32 h-32 flex items-center justify-center mb-2">
      ...
      {/* Tooltip for details */}
      {hovered && (
        <div className="absolute z-10 -bottom-32 left-1/2 -translate-x-1/2 w-56 bg-popover border text-popover-foreground rounded shadow-lg p-2 text-xs animate-fade-in">
          <div><span className="font-semibold">Relation:</span> N/A</div>
          <div><span className="font-semibold">Status:</span> <span className={member.isDeceased ? 'text-destructive' : 'text-primary'}>{member.isDeceased ? 'Deceased' : 'Alive'}</span></div>
